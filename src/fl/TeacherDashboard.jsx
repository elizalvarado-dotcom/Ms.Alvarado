        ...
        {UNITS.map(unit => (
          <div key={unit.id} style={{ marginBottom: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              <div style={{ ...S.moduleIcon, width: '40px', height: '40px', fontSize: '1.1rem', background: unit.colorDim, border: `1px solid ${unit.colorBorder}` }}>{unit.icon}</div>
              <div style={{ ...S.moduleTitle, color: unit.color }}>{L(unit.title, lang)}</div>
            </div>

            {unit.modules.length === 0 ? (
              <p style={{ ...S.lessonPara, color: '#5b6b62', fontStyle: 'italic' }}>{t.noClassworkYet}</p>
            ) : (
              unit.modules.map(mod => (
                <div key={mod.id} style={{ marginBottom: '24px' }}>
                  <p style={{ ...S.moduleTitle, fontSize: '1rem', marginBottom: '10px' }}>{L(mod.title, lang)}</p>
                  {mod.quiz.map((q, i) => (
